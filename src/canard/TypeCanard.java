...
public enum TypeCanard {
    ...
    public static double getMultiplicateur(TypeCanard attaquant, TypeCanard cible) {
        ...
        if (attaquant == EAU && cible == FEU) return fort;
        if (attaquant == FEU && cible == GLACE) return fort;
        if (attaquant == GLACE && cible == VENT) return fort;
        if (attaquant == VENT && cible == EAU) return fort;

        if (attaquant == FEU && cible == EAU) return faible;
        if (attaquant == GLACE && cible == FEU) return faible;
        if (attaquant == VENT && cible == GLACE) return faible;
        if (attaquant == EAU && cible == VENT) return faible;
